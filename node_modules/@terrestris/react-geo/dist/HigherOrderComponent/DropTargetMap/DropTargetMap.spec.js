"use strict";

var _react = _interopRequireDefault(require("react"));

var _TestUtil = _interopRequireDefault(require("../../Util/TestUtil"));

var _DropTargetMap = require("./DropTargetMap");

var _FileUtil = _interopRequireDefault(require("@terrestris/ol-util/dist/FileUtil/FileUtil"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function _typeof(obj) { if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { _typeof = function _typeof(obj) { return typeof obj; }; } else { _typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return _typeof(obj); }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

function _possibleConstructorReturn(self, call) { if (call && (_typeof(call) === "object" || typeof call === "function")) { return call; } return _assertThisInitialized(self); }

function _assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }

function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }

function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function"); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, writable: true, configurable: true } }); if (superClass) _setPrototypeOf(subClass, superClass); }

function _setPrototypeOf(o, p) { _setPrototypeOf = Object.setPrototypeOf || function _setPrototypeOf(o, p) { o.__proto__ = p; return o; }; return _setPrototypeOf(o, p); }

describe('onDropAware', function () {
  var EnhancedComponent;
  /* eslint-disable require-jsdoc */

  var MockComponent =
  /*#__PURE__*/
  function (_React$Component) {
    _inherits(MockComponent, _React$Component);

    function MockComponent() {
      _classCallCheck(this, MockComponent);

      return _possibleConstructorReturn(this, _getPrototypeOf(MockComponent).apply(this, arguments));
    }

    _createClass(MockComponent, [{
      key: "render",
      value: function render() {
        return _react["default"].createElement("div", null, "A mock Component");
      }
    }]);

    return MockComponent;
  }(_react["default"].Component);
  /* eslint-enable require-jsdoc */


  beforeEach(function () {
    EnhancedComponent = (0, _DropTargetMap.onDropAware)(MockComponent);
  });
  describe('Basics', function () {
    it('is defined', function () {
      expect(_DropTargetMap.onDropAware).not.toBeUndefined();
    });
    it('can be rendered', function () {
      var wrapper = _TestUtil["default"].mountComponent(EnhancedComponent, {
        map: _TestUtil["default"].createMap()
      });

      expect(wrapper).not.toBeUndefined();
      expect(wrapper.first().is(EnhancedComponent)).toBe(true);
    });
    it('passes through all props', function () {
      var props = {
        someProp: '09',
        map: _TestUtil["default"].createMap()
      };

      var wrapper = _TestUtil["default"].mountComponent(EnhancedComponent, props);

      var instance = wrapper.instance();
      expect(instance.props.someProp).toEqual('09');
    });
  });
  describe('#onDrop', function () {
    it('calls the FileUtil.addShpLayerFromFile when called with a zip', function () {
      var wrapper = _TestUtil["default"].mountComponent(EnhancedComponent, {
        map: _TestUtil["default"].createMap()
      });

      var file = {
        name: 'peter.zip'
      };
      var mockEvent = {
        dataTransfer: {
          files: [file]
        },
        preventDefault: function preventDefault() {}
      };
      _FileUtil["default"].addShpLayerFromFile = jest.fn();
      var mockFunction = _FileUtil["default"].addShpLayerFromFile;
      wrapper.instance().onDrop(mockEvent);
      expect(mockFunction).toHaveBeenCalledTimes(1);
      expect(mockFunction).toHaveBeenCalledWith(file, wrapper.prop('map'));
    });
    it('calls the FileUtil.addGeojsonLayerFromFile for all other files', function () {
      var wrapper = _TestUtil["default"].mountComponent(EnhancedComponent, {
        map: _TestUtil["default"].createMap()
      });

      var file = {
        name: 'peter.json'
      };
      var mockEvent = {
        dataTransfer: {
          files: [file]
        },
        preventDefault: function preventDefault() {}
      };
      _FileUtil["default"].addGeojsonLayerFromFile = jest.fn();
      var mockFunction = _FileUtil["default"].addGeojsonLayerFromFile;
      wrapper.instance().onDrop(mockEvent);
      expect(mockFunction).toHaveBeenCalledTimes(1);
      expect(mockFunction).toHaveBeenCalledWith(file, wrapper.prop('map'));
    });
  });
  describe('#onDragOver', function () {
    it('calls preventDefault on the event', function () {
      var wrapper = _TestUtil["default"].mountComponent(EnhancedComponent, {
        map: _TestUtil["default"].createMap()
      });

      var mockFunction = jest.fn();
      var mockEvent = {
        preventDefault: mockFunction
      };
      wrapper.instance().onDragOver(mockEvent);
      expect(mockFunction).toHaveBeenCalledTimes(1);
    });
  });
});